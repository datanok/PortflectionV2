...
async function debugRegistry() {
  ...
  try {
    ...
    console.log(
      `Found ${approvedComponents.length} approved components in database:`
    );
    ...
    try {
      ...
      } = require("../src/lib/componentRegistry.ts");
      const registry = await loadComponentRegistry();

      console.log(
        `Registry loaded ${Object.keys(registry).length} components:`
      );
