...
function extractPropsSchema(
  ...
  const propLines = propsInterface
    .split("\n")
...
export function validateComponentCode(
  ...
  const hasRequiredPattern = REQUIRED_PATTERNS.some((pattern) =>
    pattern.test(componentCode)
  );
  if (!hasRequiredPattern) {
    errors.push(
      "Component must be a valid React function component with export default"
    );
  }
  ...
  if (
    ...
    warnings.push(
      "Consider importing React explicitly for better compatibility"
    );
  }
...
export function generateComponentFilename(componentName: string): string {
  return (
    componentName.replace(/[^a-zA-Z0-9]/g, "").replace(/^(\d)/, "_$1") + // Ensure it doesn't start with a number
    ".tsx"
  );
}
...
export async function writeComponentFile(
  generatedFile: GeneratedComponentFile
): Promise<void> {
