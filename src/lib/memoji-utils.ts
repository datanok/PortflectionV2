...
const CLOUDINARY_BASE_URL =
  "https://res.cloudinary.com/portflection/image/upload";
const MEMOJI_FOLDER = "Portflection Assets/memojis";
 ...
 */
export function filterMemojis(
  memojis: Memoji[],
  filters: MemojiFilters
): Memoji[] {
  return memojis.filter((memoji) => {
    ...
    if (filters.search) {
      ...
      const matchesCategory = memoji.category
        .toLowerCase()
 ...
 */
export function groupMemojisByCategory(
  memojis: Memoji[]
): Record<string, Memoji[]> {
 ...
 */
export function preloadMemojiImages(
  memojis: Memoji[],
 ...
 */
export async function fetchMemojisFromCloudinary(
  options: {
    maxResults?: number;
  ...
  if (typeof window !== "undefined") {
    console.warn(
      "fetchMemojisFromCloudinary should only be called server-side"
    );
    return [];
  }
  ...
  try {
    ...
    const folder = await cloudinary.api.search(
      "memojis",
 ...
 */
export function transformCloudinaryResourceToMemoji(
  resource: any,
  index: number
): Memoji {
