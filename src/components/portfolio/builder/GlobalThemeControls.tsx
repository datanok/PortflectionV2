...
export default function GlobalThemeControls({
  ...
  // When dialog opens, reset localTheme to current theme
  useEffect(() => {
    if (isOpen) setLocalTheme(theme);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [isOpen]);

  // Only update localTheme on prop change if dialog is not open
  useEffect(() => {
    if (!isOpen) setLocalTheme(theme);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [theme]);
  ...
  const applyColorScheme = (
    ...
    const colors = COLOR_SCHEMES[scheme][mode];
  ...
  const getPaletteMode = (mode: string) => {
      ...
      <Dialog open={isOpen} onOpenChange={setIsOpen}>
        ...
        <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            ...
            <DialogDescription>
              Customize the appearance of your entire portfolio with these
                ...
                <div className="space-y-3">
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    {Object.entries(COLOR_SCHEMES).map(([name, scheme]) => {
                      const mode = getPaletteMode(localTheme.mode);
                      const colors = scheme[mode];
                      // Check if this palette is selected
                      const isSelected = Object.keys(colors).every(
                        (key) =>
                          (localTheme as any)[key] &&
                          (localTheme as any)[key].toLowerCase() ===
                            (colors as any)[key].toLowerCase()
                      );
                      return (
                        <Button
                          key={name}
                          ...
                          onClick={() =>
                            applyColorScheme(
                              name as keyof typeof COLOR_SCHEMES,
                              mode
                            )
                          }
                          className={`h-auto p-3 flex flex-col items-start gap-2 relative border-2 ${
                      ...
                      <div key={key} className="space-y-2">
                        ...
                        <div className="flex gap-2">
                          <input
                            type="color"
                            value={
                              localTheme[key as keyof GlobalTheme] as string
                            }
                            onChange={(e) =>
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    {Object.entries(SPACING_PRESETS).map(([name, spacing]) => (
                      <Button
                        ...
                        onClick={() =>
                          applySpacingPreset(
                            name as keyof typeof SPACING_PRESETS
                          )
                        }
                  ...
                  <div className="space-y-4">
                    {[
                      ...
                      <div key={key} className="space-y-2">
                        ...
                        <Slider
                          value={[
                            localTheme[key as keyof GlobalTheme] as number,
                          ]}
                          onValueChange={([value]) =>
