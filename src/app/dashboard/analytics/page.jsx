...
import {
  Eye, Share2, Globe, Bot, ArrowUpRight, ArrowDownRight,
  Download, Calendar, Filter, ArrowRight, Info, ChevronDown,
...
export default function AnalyticsDashboard() {
  ...
  useEffect(() => {
    async function fetchAnalytics() {
      try {
        ...
        const { portfolios, portfolioData, aggregatedStats, previousPeriodStats } = result;
  
        // Optional fallback if previousPeriodStats isn't returned from the API
        const previousPeriodData = previousPeriodStats || {
          totalViews: Math.floor(aggregatedStats.totalViews * (0.7 + Math.random() * 0.5)),
        ...
        if (portfolios?.length > 0) {
          const portfolioWithData = portfolios.find(p => {
  ...
  const processRecentVisitors = () => {
    const currentData = getCurrentData();
    if (!currentData?.recentVisitors || currentData.recentVisitors.length === 0) {
      return [];
    }
    ...
    return currentData.recentVisitors
      ...
      .slice(0, 5)
      .map(visitor => ({
        time: new Date(visitor.timestamp).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }),
        date: new Date(visitor.timestamp).toLocaleDateString([], { month: 'short', day: 'numeric' }),
  ...
  const humanTrafficChange = useMemo(() => {
    ...
    const currentHumans = currentData.find(d => d.name === 'Humans')?.value || 0;
    const currentBots = currentData.find(d => d.name === 'Bots')?.value || 0;
    const currentTotal = currentHumans + currentBots;
    const currentPercentage = currentTotal ? (currentHumans / currentTotal) * 100 : 0;
    
    const previousBotData = previousPeriodData.botTraffic;
    const previousHumans = previousBotData.find(d => d.isBot === false)?.count || 0;
    const previousBots = previousBotData.find(d => d.isBot === true)?.count || 0;
    const previousTotal = previousHumans + previousBots;
    const previousPercentage = previousTotal ? (previousHumans / previousTotal) * 100 : 0;
        ...
        <CardHeader className="flex flex-row items-start space-x-3 pb-2">
          ...
          <div>
            <h3 className="text-sm font-semibold">
              {errorCode === "NO_PORTFOLIOS" ? "No portfolios yet" : "Error loading analytics data"}
            </h3>
  ...
  return (
    ...
    <div className="rounded-lg shadow p-4 md:p-6 bg-background border border-border">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-4">
        <h1 className="text-2xl md:text-3xl font-bold text-foreground">Portfolio Analytics</h1>
      ...
      <div className="grid gap-4 md:gap-6 grid-cols-2 md:grid-cols-4">
        <div className="bg-card rounded-lg p-4 border border-border">
          ...
          <div className="flex items-center mb-1">
            ...
            <span className="text-xl md:text-2xl font-bold text-foreground">{getTotalViews().toLocaleString()}</span>
          </div>
          {showComparison && (
            <div className={`flex items-center text-xs ${totalViewsChange.isPositive ? 'text-green-600' : 'text-red-600'}`}>
              {totalViewsChange.isPositive ? <ArrowUpRight className="h-3 w-3 mr-1" /> : <ArrowDownRight className="h-3 w-3 mr-1" />}
        ...
        <div className="bg-card rounded-lg p-4 border border-border">
          ...
          <div className="flex items-center mb-1">
            ...
            <span className="text-xl md:text-2xl font-bold text-foreground truncate">{getTop(processCountryData(), 'country')}</span>
          </div>
        ...
        <div className="bg-card rounded-lg p-4 border border-border">
          <div className="text-sm text-muted-foreground mb-1">Top Referrer</div>
          <div className="flex items-center mb-1">
            <Share2 className="mr-2 h-5 w-5 text-primary" />
            <span className="text-xl md:text-2xl font-bold text-foreground truncate">{formatReferrer(getTop(processReferrerData(), 'referrer'))}</span>
          </div>
          <div className="text-xs text-muted-foreground flex items-center">
        ...
        <div className="bg-card rounded-lg p-4 border border-border">
          <div className="text-sm text-muted-foreground mb-1">Human Traffic</div>
          <div className="flex items-center mb-1">
            <Bot className="mr-2 h-5 w-5 text-primary" />
            <span className="text-xl md:text-2xl font-bold text-foreground">{getHumanTrafficPercentage()}%</span>
          </div>
          {showComparison && (
            <div className={`flex items-center text-xs ${humanTrafficChange.isPositive ? 'text-green-600' : 'text-red-600'}`}>
              {humanTrafficChange.isPositive ? <ArrowUpRight className="h-3 w-3 mr-1" /> : <ArrowDownRight className="h-3 w-3 mr-1" />}
    ...
    <div className="grid gap-6 md:grid-cols-2">
      <div className="rounded-lg shadow p-4 md:p-6 bg-card border border-border">
        <h2 className="text-lg font-semibold mb-4 text-foreground">Views Over Time</h2>
      ...
      <div className="rounded-lg shadow p-4 md:p-6 bg-card border border-border">
        <h2 className="text-lg font-semibold mb-4 text-foreground">Traffic by Country</h2>
        <ResponsiveContainer width="100%" height={300}>
      ...
      <div className="rounded-lg shadow p-4 md:p-6 bg-card border border-border">
        <h2 className="text-lg font-semibold mb-4 text-foreground">Top Referrers</h2>
        <ResponsiveContainer width="100%" height={300}>
          <PieChart>
            <Pie
      ...
      <div className="rounded-lg shadow p-4 md:p-6 bg-card border border-border">
        <h2 className="text-lg font-semibold mb-4 text-foreground">Bot vs Human Traffic</h2>
        <ResponsiveContainer width="100%" height={300}>
          <PieChart>
            <Pie
              data={processBotData()}
